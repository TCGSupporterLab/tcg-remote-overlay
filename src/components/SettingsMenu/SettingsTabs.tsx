...
export const SettingsTabs = ({ activeTab, onTabChange }: SettingsTabsProps) => (
    <div className="flex w-full border-b border-border/50 bg-black/20 px-[0px] pt-[0px] no-scrollbar gap-[1px]">
        {TABS.map((tab) => (
            <button
                key={tab.id}
                className={`flex-1 justify-center px-[6px] py-[6px] border-b-2 font-bold transition-colors flex items-center gap-[4px] whitespace-nowrap text-sm ${activeTab === tab.id ? 'border-primary text-white' : 'border-transparent text-gray-400 hover:text-gray-200'}`}
