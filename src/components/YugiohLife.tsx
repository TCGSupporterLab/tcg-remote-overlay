...
export const YugiohLife: React.FC<YugiohLifeProps> = ({
    isOverlay = false,
    ...
    const normalWrapperClass = "flex flex-col h-full w-full max-w-5xl mx-auto pb-4 px-4";
    ...
    const normalRowClass = "flex flex-row items-center justify-center gap-4 flex-1 animate-in fade-in duration-500 w-full";
    ...
    const getTargetStyle = (isTarget: boolean, player: 'p1' | 'p2') => {
        if (!isTarget) return {
            ...
            padding: 'calc(1.5rem + 3px)' // Compensate for 3px border difference (4px vs 1px) to prevent layout shift
        };
        return {
    ...
    return (
        <div
            className={`${isOverlay ? overlayWrapperClass : normalWrapperClass} transition-all duration-500`}
            style={isOverlay ? {
                position: 'fixed',
                inset: 0,
                zIndex: 50,
        ...
        >
            {/* Life Display Area */}
            <div className={isOverlay ? overlayRowClass : normalRowClass}>
            ...
            {/* Calculator Panel - Always visible in controller mode */}
            {!isOverlay && (
