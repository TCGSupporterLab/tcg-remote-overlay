...
- **レスポンシブ同期**: ウィンドウのサイズを変更しても表示位置や比率が崩れず、遅延なく追従します。

### 🎤 ホロライブモード
- **高速カード検索**: 1,700枚以上の全カードから名称、色、タイプ、Bloomレベルで瞬時にフィルタ。
- **ピン留め機能**: 最大10枚までのクイックアクセス。頻繁に使うカードやデッキリストを管理。
...
- **マルチウィンドウ同期**: `BroadcastChannel API` により、操作画面（コントローラー）と表示画面（オーバーレイ）を別ウィンドウに分離して管理可能。
...
1. **リポジトリをクローン**
   ```bash
   git clone https://github.com/RemoteDuelDev/RemoteDuelTool.git
   cd RemoteDuelTool
   ```
...
1. **オーバーレイを起動**: アプリ右上の「オーバーレイ起動」ボタンをクリックして専用ウィンドウを開きます。
