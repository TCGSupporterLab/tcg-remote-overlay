...
- **誤操作防止 (Action Shield & Interaction Exclusion)**:
    ...
    - **入力遮断 (Interaction Exclusion)**: 設定メニュー（`.settings-overlay`）、ビデオ調整パネル（`.video-adjustment-overlay`）、および選択アクションバー（`.selection-action-bar`）上での操作は、背後のウィジェットの選択ツール（Selecto）によって無視されます。これにより、メニュー操作中に背後のウィジェットが意図せず選択解除されたり、ドラッグが開始されたりすることを防止しています。
- **インテリジェント・ポジショニング**:
...
### 管理機能
- **保存**: アクションバーの「保存」ボタンから、現在の選択ウィジェットの状態を名前を付けて保存できます（`localStorage`）。
- **復元**: 設定メニューの「マイレイアウト」タブから適用できます。既存のウィジェットを上書きせず、レイアウトに含まれるものだけが特定の位置へ移動し、必要に応じて他のウィジェットを自動的に非表示にするオプション（「復元時に他を非表示」）を搭載。
- **外部連携**: 各レイアウトを `.json` ファイルとしてエクスポート・インポート可能です。
- **デフォルトレイアウト自動生成**: アプリ初回起動時（またはリセット時）、`src/data/default_layout/` フォルダ内の JSON ファイル群から推奨レイアウトが自動的にインポートされます。フォルダに JSON を追加するだけでプリセットが拡張される拡張性の高い設計（Viteの `import.meta.glob` を活用）を採用しています。
...
- **カテゴリ・アコーディオン**:
    - 設定項目は「汎用ウィジェット」「専用ウィジェット」「共通設定」の3つのカテゴリに分類されています。
...
- **バナーアクション (Banner Actions)**:
    ...
    - **操作の独立性**: ビデオ調整モード中であっても「右クリック」や `Esc` キーで設定メニューへのアクセスが可能です。メニューを閉じてもビデオ調整状態は維持されます。
