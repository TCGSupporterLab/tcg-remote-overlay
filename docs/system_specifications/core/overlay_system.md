...
### ビデオ調整システム
オーバーレイ窓に表示されるビデオ映像は、コントローラー窓からリモートで調整可能です。
- **調整モードの起動**: コントローラーの「ビデオ調整」ボタン、またはキーボードの `A` キー、あるいはオーバーレイ窓上での **マウスホイールクリック** で切り替えます。
- **映像の移動 (Pan)**: 調整モード中に映像をドラッグして、表示位置を自由に調整できます。
- **ズーム (Zoom)**: マウスホイールの回転で映像を拡大・縮小（1.0倍〜5.0倍）できます。
- **トリミング (Trimming)**: 調整モード中に表示される「Trimming」スライダーを使用して、上下左右の端を個別にカットできます。
- **リモート操作**: 調整モード自体はコントローラーからON/OFFを切り替えますが、実際のドラッグやズーム操作は（OBS等に映っている）オーバーレイ窓を直接触って行います。
...
- `remote_duel_video_flip`: ビデオ反転設定（none / horizontal / vertical / both）
- `remote_duel_video_crop`: ビデオの位置・ズーム・トリミング情報（JSON形式）
