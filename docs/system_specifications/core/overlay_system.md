...
## 1. オーバーレイ同期システム
本システムは、操作を行う「コントローラー窓」と、リモート対戦画面を表示する「オーバーレイ窓」の間でリアルタイムなデータ同期を行います。この仕組みにより、単一のブラウザ内での対戦はもちろん、OBS等の配信環境への統合も可能です。
...
- **同期されるデータ**:
    - 各ウィジェットの表示・非表示
    - ライフポイント、ログ、入力中の計算数値
    - 検索フィルタ、ピン留めカードリスト、現在のパス、および**表示モード状態**
    - 選択中のカード、表示番号（Shift+数字キーによる操作）
    - 映像背景設定（ソース・ズーム・回転・反転・トリミング）
    - ウィンドウ・マネージャー（単一インスタンス管理）

### 同期安定化アルゴリズム (Multi-Tab Stability)
- **無限ループ防止**: Blob URL (ObjectURL) はブラウザタブごとに異なるため、一方の窓が「修正」したURLを同期すると、他方の窓が再度「修正」して送り返す無限ループが発生する可能性があります。これを防ぐため、内部的なURL解決によるステート更新時は `skipBroadcast` フラグを使用し、ループを遮断しています。
- **ユーザー操作の優先**: フィルターの更新やカード選択などの「ユーザーによる能動的な操作」のみを全タブへブロードキャストし、安定した同期環境を実現しています。
...
- **選択アクションバー (SelectionActionBar)**:
    ...
    - **グループ化/解除**: 選択したウィジェットを瞬時にグループ化、または解除できます。解除時、誤操作防止のため対象ウィジェットの選択状態は自動的にクリアされます。
...
- **誤操作防止 (Action Shield & Interaction Exclusion)**:
    ...
    - **ZIP展開中の操作ロック (ZIP Extraction Lock)**: ZIPファイルの展開プロセス中は、誤操作や競合を防止するため、すべてのキーボードショートカット、右クリックメニュー、およびマウスによる矩形選択がグローバルに遮断されます。
- **インテリジェント・ポジショニング**:
...
- **効率的なタブ構成**:
    - 設定項目は以下の順序でタブ分けされており、アイコンによる視覚的な識別とアクセントカラーが付与されています：
        ...
        4. **操作説明**: セットアップ手順、ウィジェット操作方法、レイアウト管理のガイド。
        5. **ショートカット**: キーボードショートカットおよびマウス操作の一覧。
        6. **その他**: 免責事項と応援リンク。
- **カテゴリ・アコーディオン**:
...
## 2. 背景表示モード (補助機能: OBS統合等)
ブラウザ単独での利用に加え、配信ソフトへの取り込みを補助するため、以下の2つのモードを切り替え可能です。設定は「背景・映像」タブから行います。

- **透過なし (Normal)**: アプリ標準のダーク背景を表示。UI全体はOSの設定に関わらず、常にダークモードとして動作するように設計されています。
- **グリーンバック (Green)**: 背景を `#00ff00` に設定。クロマキー合成を想定したモード。
- **映像背景 (Video)**: カメラ映像または画面共有映像を最背面に表示。
    - **カメラデバイス選択**: 複数のカメラが接続されている場合、設定メニューの「背景・映像」タブ、または未起動時の開始画面から特定のデバイスを選択・切り替え可能です。
...
### 映像エラーハンドリング (Video Error Handling)
ユーザーが映像ソースを選択する際、ブラウザの制約やハードウェアの状況により発生する様々なエラーを検知し、解決のヒントとなる具体的な日本語メッセージを表示します。

- **エラー種別ごとの対応**:
    - **接続キャンセル (`NotAllowedError`)**: ユーザーがブラウザの許可ダイアログを閉じた、または拒否した場合。「接続がキャンセルされました。映像ソースの許可が必要です。」と表示されます。
    - **デバイス競合 (`NotReadableError`)**: 指定したカメラが既に他のアプリ（Discord, OBS等）で使用されている場合。「デバイスが既に使用されています。他のアプリを確認してください。」と表示します。
    - **デバイス未検出 (`NotFoundError`)**: カメラが物理的に接続されていない場合。「デバイスが見つかりません。カメラが正しく接続されているか確認してください。」と表示します。
- **自動切断検知**: 共有中のウィンドウが閉じられたり、カメラが抜かれたりしたことを検知（`track.onended`）すると、自動的に「接続がキャンセルされました」というメッセージと共に開始画面へ復帰します。
- **マルチライン・ガイダンス**: エラーメッセージには改行（`whitespace-pre-wrap`）が適用されており、原因と対処法を分かりやすく2行で表示します。
...
- `tcg_remote_selected_camera_id`: 選択されたカメラデバイスのID
