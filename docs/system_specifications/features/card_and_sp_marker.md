...
### 展開プロセス
- **検出とプレビュー**: ZIPファイルがドラッグ＆ドロップまたはメニューから読み込まれると、展開前に内容（ファイル数、トップレベルの構成）を解析し、確認モーダルを表示します。
    - **ディレクトリ階層の最適化**: ZIP内の全ファイルが単一のルートフォルダに格納されている場合、展開時にその冗長なトップフォルダを自動的にスキップし、中身だけを抽出します。
- **自動サブフォルダ作成**: ユーザーが選択した「展開先フォルダ」の直下に、**ZIPファイル名（拡張子なし）に基づいた新しいフォルダ**を自動作成し、そこへ展開します。
...
- **多重実行防止**: 巨大なアーカイブの展開中に重複した要求が発生しないよう、状態ガード (`useRef`) を用いて直列化されています。
