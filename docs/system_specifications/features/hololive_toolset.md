...
- **動的フィルタ項目の生成**:
    ...
    - **データの自動補完**: 定義ファイルに記述がないプロパティであっても、ロードされたカードデータから自動的に属性を抽出し、フィルタ項目として追加します。
- **カラーフィルタ (色)**:
...
- **ライブラリモード (Library Mode)**:
    ...
    - **フォルダキャッシュ**: 最後に使用したフォルダ（最大5件）の状態（ピン留め、選択中カード、階層）を個別に記憶しており、フォルダを切り替えるだけで以前の作業状態が復元されます。
...
- **キーボードナビゲーション**:
    ...
    - **機材操作 (`.`)**: テンキーの「.」を押すと、即座にダイスを振ります（ホロライブモードではコイン判定の待機はありません）。
