...
## 概要
TCG Remote Overlayは、リモート環境でのカードゲーム対戦をサポートするためのWebアプリケーションです。あらゆるTCGで利用可能な汎用ツールとして設計されており、特に「遊戯王OCG」や「ホロライブOCG」向けには専用の支援機能も提供しています。
ライフポイントの管理、ダイス・コインのシミュレーション、カードデータの検索、およびリモート対戦画面を構築するためのオーバーレイ機能を提供します。あらゆるTCGで汎用的に利用可能です（OBS等の配信ソフトへの統合も補助的にサポート）。

## ドキュメント構成

### 1. [コアシステム仕様 (Core System)](./core/overlay_system.md)
- オーバーレイ同期（BroadcastChannel）
- 背景モード（Normal / Green / **映像背景・カメラデバイス選択**）
- 映像調整（ズーム、パン、トリミング同期）
...
- **ZIPファイル直接展開（自動解析・プレビュー・サブフォルダ作成・接続）**
- **Windows環境への最適化（パス正規化、禁止文字サニタイズ、安全なファイル操作）**
