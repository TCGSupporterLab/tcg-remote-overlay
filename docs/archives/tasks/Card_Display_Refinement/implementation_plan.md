...
- **対象**: 第1階層に表示されるフォルダ。
- **挙動**: 
    - フォルダをピン留めした場合、そのフォルダ直下にあるすべてのカード（第2階層の実体ファイル）を一括でピン留めリストに追加します。
    - フォルダのピン留めを解除した場合、そのフォルダ直下にあるすべてのカードをリストから削除します。
- **表示**: 第1階層のフォルダにもピンバッジを表示します。
    - フォルダ内のすべてのカードがピン留めされている場合に「アクティブ」なバッジを表示。
    - それ以外（一部またはなし）の場合は「非アクティブ」なバッジを表示。
- **制限**: フィルタリング設定に基づき、第3階層以降のカードは処理の対象外とします。
