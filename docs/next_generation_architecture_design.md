...
- **結合機能**: 複数のウィジェットを一つのグループとして固定し、一括操作を可能にする。
  - 実装方針: 複数選択（Ctrl+クリックまたは矩形ドラッグ）された状態から「結合（Group）」操作を行うことで、1つの `WidgetGroup` を生成する。
  - **フラットな構造**: グループの入れ子（グループのグループ化）は行わず、既存のグループを含む選択を再結合した場合は、古いグループを解体して新しい1つのグループに統合（フラット化）する。
