...
3.  **デザインの洗練（角丸・白エッジ）**: カード枠の角丸を `3xl` (24px) に強化し、よりモダンな印象にしました。また、枠のエッジに薄い白系のボーダー (`rgba(203, 213, 225, 0.4)`) を追加し、OBSでの合成時でもカードの輪郭がはっきりと際立つように改善しました。
4.  **回転**: `rotated` モード時、テキストベースのレイアウトも正しく 180 度回転して表示されます。
5.  **カード種別ごとの表示**:
    - 推しホロメンのスキルが正しく表示されること。
    - ホロメンのアーツコストやダメージが正しく表示されること。
    - サポートカードの能力テキストが表示されること。
