...
- テキストモード時の専用レイアウトを実装しました：
    - **ヘッダーレイアウトの最適化**: 
        ...
        - **表示情報の洗練**:
            - **Extra文字サイズの統一**: カード最下部の注記（Extra）セクションの文字サイズを、通常のテキストと同じ `text-xs` に引き上げ、視認性を向上させました。
            - **LIMITED重複の除去**: 能力説明欄（abilityText）の先頭に「LIMITED」という文言が残っている場合、それを自動的に非表示にする処理を追加しました。これにより、下部の専用LIMITEDラベルとの重複を避け、よりクリーンな表示を実現しました。
            - **タグの表示**: カード枠の最下部左側に、カードに設定されているタグ（`#EN`, `#Justice` 等）を青色のハッシュタグ形式で表示するようにしました。
        - **用語の修正**: 推しスキルの「コスト」表記を、公式用語に合わせて「**ホロパワー**」に変更しました。
        - **サポートカードの表記の簡略化**: カードタイプから「サポート・」や「LIMITED」を削除し、**「アイテム」「イベント」などのカテゴリのみ**を表示するようにしました。
        - **LIMITED表示の最適化**: 種別欄から「LIMITED」を削除する代わりに、カード最下部のExtraセクション（ホロメンの注記と同じ場所）に「**LIMITED: ターンに1枚しかつかえない**」と明示的に表示するようにしました。
        - **表示情報の集約**: 重複していたBloomレベルの小型バッジを削除し、大型化されたヘッダー上部のラベル（`text-xl`）に情報を集約することで、ノイズを減らしつつ視認性を最大化しました。
        - **大文字変換の解除**: ヘッダーラベルの `uppercase` 指定を削除し、`Debut`, `1st` 等の混合表記を維持するようにしました。
        - **ホロメンの表記変更**: 「ホロメン」という共通の種別表記を廃止し、**Bloomレベル (Debut/1st/2nd)** を直接表示するようにしました。Buzzカードの場合は、レベルの後に **" Buzz"** が自動的に付与されます。
        - **カラーコーディング (強化)**: CSSクラスの競合を防ぐため、カード名の文字色を**インラインスタイル (`style={{ color: textColor }}`)** で直接指定するように強化しました。
        - **判定ロジックの改善**: 単純な完全一致ではなく、多色カードやデータの微細なゆれにも対応できるよう `includes` による部分一致判定を採用しました。
        - **クロマキー配慮**: 緑色属性については、引き続きエメラルドグリーンを採用し、視認性を確保しています。
